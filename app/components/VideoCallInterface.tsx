...
import { Video, VideoOff, Mic, MicOff, PhoneOff, Loader2, Notebook } from "lucide-react";
...
export default function VideoCallInterface({ onEndCall }: VideoCallInterfaceProps) {
    ...
    return (
        <div className="relative w-full h-full flex bg-slate-900 rounded-3xl overflow-hidden">
            {/* Main Video Area */}
            <div className={`relative flex-1 h-full transition-all duration-300 ${showNotes ? 'pr-80 sm:pr-96' : ''}`}>
                ...
                {connected && !error && (
                    <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 z-20">
                        ...
                        <button
                            ...
                            className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${showNotes
                ...
                {/* Status Indicator */}
                {connected && !error && (
                    <div className="absolute top-6 left-6 flex items-center gap-2 bg-slate-900/60 backdrop-blur-sm px-4 py-2 rounded-full z-10">
